...
**看得见的组：「🌬️ 御风 | 断流」**——御风栈的五式大多是 TUN/DNS 底层配置（不产生策略组），唯独国外 QUIC 断流是路由可控的，特意做成**紧跟总卷的可见策略组**，面板里随时切三档：
...
| 国外 QUIC 断流 | **🌬️ 御风 \| 断流** | QUIC 急流一道闸，或截或放一目了然（御风栈唯一可见组） |
...
| `屏蔽国外QUIC` | 「🌬️ 御风 \| 断流」的初始默认档：true=REJECT 拦截 / false=PASS-RULE 放行（面板可随时切档） |
