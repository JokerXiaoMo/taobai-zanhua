...
### 2026-09 命名与图标重做（本版自研，非上游变更）

- **命名重做**：策略组一律改为标准功能名 / 服务名——「🍑 桃印 | 总卷」→「桃白簪花 · 主控」、「🌸 寻花 | 自动」→「自动选择」、「🏠 归檐 | 直连」→「直连」、「🌙 拾遗 | 兜底」→「兜底」、「🌬️ 御风 | 断流」→「QUIC 断流闸」、地区组改为「🇯🇵 日本」等，19 个服务组改为「YouTube」「Google FCM」「Meta」等标准服务名
- **图标重做**：全套图标改为自研「簪花印」（38 枚 SVG 存于 `Icons/`），脚本 44 处引用全部指向本仓库，不再引用第三方图标集
- **主视觉重做**：`docs/assets/hero.webp` 换为中式水墨长卷
...
同步原则：**分流逻辑、规则集、DNS 处理全部跟随上游**；仅策略组 / 地区命名保留桃白簪花自有命名，四项增强（`sniffer`、`global-client-fingerprint`、30s 保活、50ms 容差自动选择）保持不变。

### 最近一次同步：`9c01b6e` → `9b9f2cc`（4 笔）

| 提交 | 说明 | 桃白簪花落地 |
| :--- | :--- | :--- |
| `18e40e1` | feat: 新增 Meta、Line 策略组 | 原「影格」组升级为 **「Meta」**：规则集由 `instagram` 换成 `meta`（`facebook` 域名集），并补一个 `facebook_ip`（ipcidr，带 `no-resolve`），域名 + IP 双覆盖；同时新增 **「LINE」**（`line` 域名集）。开关、组名、规则目标三处命名已一起改名，`ruleOptionsEnable` 键名同步，避免上游「改名键失配 → 静默失效」的老坑 |
| `44ce58e` | perf: 优化DNS配置 | `chinaDNS` 去掉首位的 `'system'`，回到「阿里 + 腾讯」两个国内 DNS；`direct-nameserver` / `nameserver-policy['rule-set:cn']` 随之生效 |
| `be02e39` | feat: EHentai 增加 `直连` 选项 #42 | 「E-Hentai」加上 `direct: true`，成员表补入「直连」，方便把 E 站直接走本地网络（默认仍指向「🇺🇸 美国」） |
...
| 提交 | 说明 | 桃白簪花落地 |
| :--- | :--- | :--- |
| `188ceec` | feat: add microsoft_ip、apple_ip | 「Microsoft」「Apple」新增 GeoIP 规则集与 `no-resolve` 规则：域名与 IP 双覆盖，命中更彻底 |
...
| `3466ca3` | 将 googlefcm 添加到 fake-ip-filter | 「Google FCM」开关开启时，`rule-set:googlefcm` 会进入 `fake-ip-filter`，避免 FCM 推送被 fake-ip 干扰 |
| `fefcb66` | docs: add Star History | README 文档改动，本仓库自行维护文档，未同步 |

> ⚠️ 本次同步踩到一个「上游键名 vs 本版键名」的坑：上游新增代码写的是 `ruleOptionsEnable['FCM']`，而本版把该开关改名为「Google FCM」，直接照搬会导致新功能**静默失效**。已改为 `ruleOptionsEnable['Google FCM']`，并加了专项测试守护。
